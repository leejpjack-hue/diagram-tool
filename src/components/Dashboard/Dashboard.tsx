...
type IconName = 'home' | 'clock' | 'star' | 'template' | 'boards' | 'folder' | 'trash' | 'activity' | 'search' | 'plus' | 'upload' | 'storage' | 'menu' | 'grid' | 'list' | 'filter' | 'more' | 'present' | 'download' | 'copy' | 'edit' | 'history' | 'chevron' | 'close' | 'check' | 'command' | 'share';
...
export function Dashboard({ onOpen, onCreate, onPublish, notify }: DashboardProps) {
  ...
  const [importReport, setImportReport] = useState<{ title: string; skipped: Array<{ kind: string; reason: string }> } | null>(null);
  ...
  const handleImport = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setCreateOpen(false);
      notify('success', imported.boards.length === 1 ? `Imported “${imported.boards[0].title}”` : `Imported ${imported.boards.length} boards`);
      if (imported.skipped.length > 0) {
        setImportReport({
          title: imported.boards.length === 1 ? `Imported “${imported.boards[0].title}”` : `Imported ${imported.boards.length} boards`,
  ...
  return (
    <div className="flex min-h-0 flex-1 overflow-hidden bg-white text-slate-900" onClick={() => setMenuFor(null)}>
      ...
      {shareBoard && <ShareDialog board={shareBoard} onClose={() => setShareBoard(null)} notify={notify} />}
...
function ImportReportDialog({ report, onClose }: { report: { title: string; skipped: Array<{ kind: string; reason: string }> }; onClose: () => void }) {
  return (
    <Modal title="Import complete" subtitle={report.title} onClose={onClose} width="max-w-lg">
      <div className="space-y-4 p-5" data-testid="import-report">
        {report.skipped.length === 0 ? (
          <p className="text-sm text-slate-600">Every supported board, Space, template, and deck object was restored.</p>
        ) : (
          <div>
