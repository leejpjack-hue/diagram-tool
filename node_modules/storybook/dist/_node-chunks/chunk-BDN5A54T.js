...
} from "./chunk-ZR6GQ4IP.js";
import {
  globalSettings
} from "./chunk-2CDWZI34.js";
import {
  resolvePackageDir
} from "./chunk-3CFMGL7P.js";
import {
  slash
} from "./chunk-R7ECNCMP.js";
import {
  __commonJS,
  __toESM
...
var anonymousProjectId, getProjectSinceResult, getAnonymousProjectId = () => {
  ...
  try {
    let projectRootPath = relative(getProjectRoot3(), process.cwd()), result = executeCommandSync({
      command: "git",
...
function detectExplicitAgent(env) {
  if (env.AGENT === "amp")
    return {
      name: "amp"
    };
...
var detectAgent = (options) => {
  let env = options.env, explicit = detectExplicitAgent(env);
  if (explicit)
    return explicit;
  if (options.stdoutIsTTY)
    return;
  let isDumbTerm = env.TERM === "dumb", hasAgentPager = env.GIT_PAGER === "cat";
...
}, inCI = isCI2(), agentDetection = detectAgent({ stdoutIsTTY: process.stdout.isTTY, env: process.env }), globalContext = {
  inCI,
