import CJS_COMPAT_NODE_URL_i0f2i4nx7e from 'node:url';
import CJS_COMPAT_NODE_PATH_i0f2i4nx7e from 'node:path';
import CJS_COMPAT_NODE_MODULE_i0f2i4nx7e from "node:module";

var __filename = CJS_COMPAT_NODE_URL_i0f2i4nx7e.fileURLToPath(import.meta.url);
var __dirname = CJS_COMPAT_NODE_PATH_i0f2i4nx7e.dirname(__filename);
var require = CJS_COMPAT_NODE_MODULE_i0f2i4nx7e.createRequire(import.meta.url);
...
import {
  toEstree
} from "../_node-chunks/chunk-OHJO7K5J.js";
import {
  StatusTypeIdMismatchError
} from "../_node-chunks/chunk-26ZNYBCV.js";
import {
  debounce,
  generateStoryFile,
  getErrorLevel,
  ...
  useStatics,
  withTelemetry
} from "../_node-chunks/chunk-WGZQUMPK.js";
import {
  detectPnp
} from "../_node-chunks/chunk-FDCY4D4J.js";
import "../_node-chunks/chunk-EQ54US4S.js";
import {
  StorybookError
} from "../_node-chunks/chunk-DGG37MUT.js";
import {
  telemetry
} from "../_node-chunks/chunk-BDN5A54T.js";
import "../_node-chunks/chunk-WCGLVU7G.js";
import {
...
} from "../_node-chunks/chunk-CUCNHZNT.js";
import "../_node-chunks/chunk-3BKNA4JV.js";
import "../_node-chunks/chunk-4X2SJGHU.js";
import {
  require_pretty_hrtime
} from "../_node-chunks/chunk-AEHREA72.js";
import "../_node-chunks/chunk-ZR6GQ4IP.js";
import "../_node-chunks/chunk-2CDWZI34.js";
import {
  invariant,
  up2 as up
} from "../_node-chunks/chunk-S447H5QO.js";
import "../_node-chunks/chunk-2WRPPG3L.js";
import {
  importModule,
  resolvePackageDir
} from "../_node-chunks/chunk-3CFMGL7P.js";
import {
  dirname,
  join,
  relative,
  resolve
} from "../_node-chunks/chunk-DVLFTQPE.js";
import {
  slash
} from "../_node-chunks/chunk-R7ECNCMP.js";
import "../_node-chunks/chunk-TMH7JVOQ.js";
import {
  Tag
} from "../_node-chunks/chunk-TZR6DQXN.js";
import {
  require_dist
} from "../_node-chunks/chunk-J3D4UFKB.js";
import {
  require_lib,
  require_src
} from "../_node-chunks/chunk-4LRNR45C.js";
import {
  require_picocolors
} from "../_node-chunks/chunk-42F4BXGV.js";
import {
  __commonJS,
  __require,
  __toESM
} from "../_node-chunks/chunk-4BS6LSRK.js";
...
async function getManagerBuilder() {
  return await import("../_node-chunks/builder-manager-42KH6DCX.js");
}
  ...
  async function onChangeOrRemove(absolutePath, removed) {
    ...
    !removed && isDirectory(absolutePath) && await Promise.all(
      specifiers.filter((specifier) => importPath.startsWith(specifier.directory)).map(async (specifier) => {
        ...
        ), { globby } = await import("../_node-chunks/globby-ODT6OUUO.js");
...
import { dirname as dirname2, extname, join as join5, normalize, relative as relative4, resolve as resolve3, sep } from "node:path";
...
var getAttr = (elt, what) => elt.attributes.find((n) => n.type === "JSXAttribute" && n.name.name === what), getAttrValue = (elt, what) => getAttr(elt, what)?.value, getAttrLiteral = (elt, what) => {
  ...
  let store = {
    ...
    metaTags: void 0,
    imports: void 0
  }, { compile } = await import("../_node-chunks/mdx-N42X6CFJ-X7ZYXT7S.js");
  await compile(code, {
...
var makeAbsolute = (otherImport, normalizedPath, workingDir) => otherImport.startsWith(".") ? slash(resolve3(workingDir, normalizeStoryPath(join5(dirname2(normalizedPath), otherImport)))) : otherImport, StoryIndexGenerator = class _StoryIndexGenerator {
  ...
  static async findMatchingFiles(specifier, workingDir, ignoreWarnings = !1) {
    ...
    if (cached)
      return cached;
    let pathToSubIndex = {}, globCwd = slash(resolve3(workingDir, specifier.directory)), globPattern = specifier.files.startsWith("!") ? `./${specifier.files}` : specifier.files, { globby } = await import("../_node-chunks/globby-ODT6OUUO.js"), files = await globby(globPattern, {
      absolute: !0,
      cwd: globCwd,
      ...commonGlobOptions2(globPattern)
    });
    return files.length === 0 && !ignoreWarnings && once.warn(
      `No story files found for the specified pattern: ${import_picocolors3.default.blue(
        join5(specifier.directory, specifier.files)
      )}`
  ...
  async extractStories(specifier, absolutePath, projectTags = []) {
    ...
    let toImportPath = (path2) => path2 ? path2.startsWith("virtual:") ? path2 : slash(normalizeStoryPath(relative4(this.options.workingDir, path2))) : importPath, storyEntries = indexInputs.map((input) => {
      ...
      if (result.of) {
        ...
        dependencies.forEach((dep) => {
          if (dep.entries.length > 0) {
            ...
            normalize(resolve3(this.options.workingDir, first.importPath)).startsWith(
  ...
  async getPreviewCode() {
    let previewFile = ["js", "jsx", "ts", "tsx", "mjs", "cjs", "mts"].map((ext) => join5(this.options.configDir, `preview.${ext}`)).find((fname) => existsSync(fname));
...
async function buildStaticStandalone(options) {
  ...
  let [features, core2, staticDirs] = await Promise.all([
...
// src/core-server/utils/doTelemetry.ts
async function doTelemetry(app, core2, storyIndexGeneratorPromise, options) {
...
// src/core-server/utils/validate-websocket-token.ts
import { timingSafeEqual } from "node:crypto";
function isValidToken(token, expectedToken) {
  if (!token || !expectedToken)
    return !1;
  let a = Buffer.from(token, "utf8"), b = Buffer.from(expectedToken, "utf8");
  try {
...
var ServerChannelTransport = class {
  constructor(server, token) {
    this.token = token, this.socket = new WebSocketServer({ noServer: !0 }), server.on("upgrade", (request, socket, head) => {
      if (request.url) {
...
function getServerChannel(server, token) {
  let transports = [new ServerChannelTransport(server, token)], channel = new Channel({ transports, async: !0 });
...
async function storybookDevServer(options) {
  let [server, core2] = await Promise.all([getServer(options), options.presets.apply("core")]), app = build_default2({ server });
  assert(core2?.channelOptions?.wsToken, "wsToken is required for securing the server channel");
  let serverChannel = await options.presets.apply(
    "experimental_serverChannel",
    getServerChannel(server, core2.channelOptions.wsToken)
  ), workingDir = process.cwd(), configDir = options.configDir, stories = await options.presets.apply("stories"), normalizedStories = normalizeStories2(stories, {
    configDir,
    workingDir
  ...
  options.debugWebpack && logConfig2("Preview webpack config", await previewBuilder.getConfig(options)), core2?.disableProjectJson || useStorybookMetadata(app, options.configDir);
